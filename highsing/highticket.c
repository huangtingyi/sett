	...
	struct PstnDetailTicketStruct *ptHead)
{
	...
	while(ptHead!=NULL){
		...
		while(ptLoadCur!= NULL){
			...
			printf("sCallingNbr=%s,toll_fee=%d,sdr=%s\n",ptCur->sCallingNbr,ptCur->iTollFee,ptCur->sSdr);
...
/*20140905修改，由于前期碰到费率为0，所以drate=(Temp.iTollFee*1.0/100)/atof(sRateRes)出现异常，先默认为9.37814*/
	if(atof(sRateRes)==0) strcpy(sRateResT,"9.37814");
	else strcpy(sRateResT,sRateRes);

	drate=(Temp.iTollFee*1.0/100)/atof(sRateResT);
