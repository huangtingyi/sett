...
/*将FCdma1x结构转换为,JoinCdr结构*/
void InitJoinCdr(struct FileControlStruct *pTemp,struct JoinCdrStruct *po)
{/*数据文件移动到内存部分*/

	char sTemp[15];
	struct FCdma1xTicketStruct *pi=(struct FCdma1xTicketStruct*)pTemp->pData;
	
	bzero((void*)po,sizeof(struct JoinCdrStruct));

	strncpy(po->sCallType,		pi->sRecordType,2);
	strncpy(po->sMsisdn,		pi->Mdn,15);
	strncpy(po->sOtherParty,	pi->sNai,24);

	strncpy(sTemp,pi->sStartTime,14); sTemp[14]=0;
	po->tStartTime=tGetTime(sTemp);
	
	strncpy(sTemp,pi->sStopTime,14); sTemp[14]=0;
	
	po->iDuration=	tGetTime(sTemp)-po->tStartTime;
	
	strncpy(sTemp,pi->sDataCountTermination,10);sTemp[10]=0;
	po->iRoamFee=	atoi(sTemp);
	
	strncpy(sTemp,pi->sDataCountOrigination,10);sTemp[10]=0;
	po->iTollFee=	atoi(sTemp);

	po->iAddFee=	0;

	/*这里暂时取UserType字段*/
	strncpy(po->sDeformFlag,	pi->sUserType,1);
...
void GetAppSeqName(char sAppSeqName[])
{
	...
	sprintf(sAppSeqName,"%s%s/joinc1x.seq",sAppHome,"ctl");
}
/*
输入：
	source_dir下的 *.J的一组文件，
	JC1XING.CDR（未完成合并的话单）文件.
输出：
	target_dir下的 JC1XEDYYYYMMDDNNNNN.FIX.BIL.B文件，
	和JC1XING.CDR（未完成合并的话单）文件
功能:	
	为了安全避免重单，在处理文件前将所有的文件名改名为*.J.tmp
	将输入的文件 *.J的文件加载到内存，并将JC1XING.CDR也加载到BIN树内，
	加载话单的数量为100万条为上线，先加载JC1XING.CDR话单.
	
	检察待合并的话单，将合并后的话单,deform_flag打为,'0'，
	被合并调的话单的,deform_flag打为'X'
	
	遍历整个树，将未合并的话单写到JC1XING.CDR文件中，将合并后的话单写到
	JC1XEDYYYYMMDDNNNNN.FIX.RAT.B文件中。
	
	检察JC1XING.CDR文件大小，如果大于50万则退出;
...
/*如果有积压的上次的文件，则改名为一个临时文件，加载到一起处理,
	命名将JOIN=>JC1X*/	
	sprintf(sStackName,"%sJC1XING.CDR",sSourcePath);
	sprintf(sTempName,"%sJC1XTP%s%05d.FIX.RAT.J",
...
/*生成输出文件名*/
	sprintf(sTargetName,"%sJC1XED%s%05d.FIX.RAT.B",
		sTargetPath,sHostDate,iSeq);
	sprintf(sDoneName,"%sJC1XDN%s%05d.FIX.RAT.O",
		sTargetPath,sHostDate,iSeq);
