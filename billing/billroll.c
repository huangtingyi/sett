...
{
	int i,l;
	char sRecord[1024],msg[1024],sTargetName[256];
	char sSplitNew[21],sOldSplit[21];
	
	strcpy(sSplitNew,"");

	/*重新后TICKET_TYPE_ID<=0则不输出文件*/
	if(pn->iTicketTypeID>0){
...
/*得到重批之前的文件名后缀*/
	/*重新前TICKET_TYPE_ID<=0则不输出文件*/
	if(po->iTicketTypeID>0){
		/*默认取SPLIT_TYPE*/
		strcpy(sOldSplit,po->sSplitType);
	
		/*如果能从TICKET_TYPE_ID中取，则从TICKET_TYPE_ID中取*/	
		if(po->iTicketTypeID>=1000&&po->iTicketTypeID<9999)
			sprintf(sOldSplit,"%d",po->iTicketTypeID/1000);
	
		/*如果能从文件名取到则从文件名中取*/
		l=strlen(po->sSourceID);
		if(l>0&&po->sSourceID[l-2]=='.'&&
			po->sSourceID[l-1]!='0'&&
			po->sSourceID[l-1]!='A')
			strcpy(sOldSplit,po->sSourceID+l-1);
	
		l=PrintTicketDel(pn,po,sRecord);
		if(Item2OutListX(&pOutput,sOldSplit,sRecord,l,sSplitNew,msg)<0){
...
int main(int argc,char *argv[])
{
	...
	int iRet,iCnt=0,iMissCnt=0,iRightCnt=0,iBillFlag,iWantOutput=false;
	char sVisitAreaCode[10],sSplitNew[30],sUserType[40],sSplitType[10];
	...
	if(argc!=9){
		printf("Usage %s tablename wherecause usertype \
...
/*设置billingflag*/
		if(sSplitType[0]>='0'&&sSplitType[0]<='9')iBillFlag=false;
		else	iBillFlag=true;

		if((iRet=AnalyzeTicketBase(p,sVisitAreaCode,iBillFlag))<0)
			continue;
/*分析必要的话单字段*/
...
/*
考虑到在前一次批价和重批这段时间内，用户可能调整分离模板，导致一些
话单无法匹配上分离模板，对于这种情况，则将这些话单的SPLIT_RULE_ID设
为0标识,话单类型为-1*/
		...
		ModifyTicketFee(sTemplateRule,iBillFlag,sModule,sSplitType,p);

/*比较批价前后的话单属性,如果向符合,略过*/

		if(cmitem_mobbill(p,&MobTemp)==0&&
			cmitem_msrn(&MobTemp,p)==0&&
...
/*将记录更新到数据库中*/
		if(EUpdateStructToUSttTicket(&TempUStt,false,&TempUp)<0){
			WriteAlertMsg("表%s更新数据错误.",TempUp.sTableName);
			return -1;
		}
		if((++iRightCnt)%10000==0){
			if(EUpdateStructToUSttTicket(NULL,true,&TempUp)<0){
				WriteAlertMsg("表%s更新数据错误.",TempUp.sTableName);
				return -1;
			}
			CommitWork();
...
/*		strncpy(po->sSplitRuleID,	pi->sSplitRuleID,4);*/
		...
		strcpy(po->sRoamTariffType,	pi->sRoamTariffType);
		strcpy(po->sTollTariffType,	pi->sTollTariffType);
		strcpy(po->sAddTariffType,	pi->sAddTariffType);
		strcpy(po->sMsrn,		pi->sMsrn);
...
*/	
	if((res=strcmp(pSource->sRoamTariffType,pTarget->sRoamTariffType))!=0) return res;
	if((res=strcmp(pSource->sTollTariffType,pTarget->sTollTariffType))!=0) return res;
	if((res=strcmp(pSource->sAddTariffType,pTarget->sAddTariffType))!=0) return res;


	if((res=strcmp(pSource->sUserType,pTarget->sUserType))!=0) return res;
	if((res=strcmp(pSource->sCal,pTarget->sCal))!=0) return res;
...
int cmitem_msrn(struct MobTicketStruct *pSource,
	struct MobTicketStruct *pTarget)
{
	int res=0;
/*
msrn网间结算填的是本方处理过的号码
*/
	if((res=strncmp(pSource->sMsrn,pTarget->sMsrn,12))!=0) return res;
...
/*其他关键字*/
		...
		strcpy(po->sRoamTariffType,	pm->sRoamTariffType);
		strcpy(po->sTollTariffType,	pm->sTollTariffType);
		strcpy(po->sAddTariffType,	pm->sAddTariffType);
		strncpy(po->sMsrn,		pm->sMsrn,12);
		strcpy(po->sCal,		pm->sCal);		
		strcpy(po->sUserType,		pm->sUserType);		
}
