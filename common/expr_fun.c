...
int MyIdentifyNext(char c1,char c2)
{
        /*如果是汉字就不检测下一个字符*/
...
int MyLexIdentify(char sInput[],int *piPos,char sOutput[])
{
	...
	if(IdentifyBegin(sInput[i])||sInput[i]==':'){
		...
		while(MyIdentifyNext(sInput[i-1],sInput[i])){
...
#define IDENTIFY_TYPE_OTHER1		6	/*分隔符或运算符关键字*/
/*其他复杂类型关键字，带'.'的关键字 CPX,不带'.'关键字 SPL*/

struct MyIdentifyStruct
{
	struct MyIdentifyStruct *pNext;
	struct MyIdentifyStruct *pHead;		/*只有标识符是表、字段、SEL操作时有效,
						   如果标识符是表时，其指向对这个表操作的操作符
						   如果标识符是字段时，其指向这个字段归属的表，
						   或指向这个字段对应的SEL操作符
						   如果标识符是操作符，则指向其上一级操作符*/

	struct MyIdentifyStruct *pTab;		/*本次操作对应的表*/
	struct MyIdentifyStruct *pCol;		/*本次操作表对应的字段*/
	char sIdentify[128];
	char sSubType[4];
	int iType;
};
...
int MyGetIdentifyType(char sTemp[],char sSubType[4])
{
	...
	if(!IdentifyBegin(sTemp[0])){
		strcpy(sSubType,"SEP");
		return IDENTIFY_TYPE_OTHER1;
	}
	...
	if(strncmp("FROM",sTemp,4)==0||
		...
		strncmp("UNION",sTemp,5)==0||
		strncmp("ALL",sTemp,3)==0||
		strncmp("AND",sTemp,3)==0||
		strncmp("INTERSECT",sTemp,9)==0||
		strncmp("DISTINCT",sTemp,8)==0){                 
...
void AppendIdentifyToTabList(struct MyIdentifyStruct *pTemp,struct MyIdentifyStruct *ptCur)
{
	if(pTemp==NULL) return ;

	while(pTemp->pTab!=NULL)
		pTemp=pTemp->pTab;
	
	ptCur->pTab=NULL;
	pTemp->pTab=ptCur;
}
void AppendIdentifyToColList(struct MyIdentifyStruct *pTemp,struct MyIdentifyStruct *ptCur)
{
	if(pTemp==NULL) return ;

	while(pTemp->pCol!=NULL)
		pTemp=pTemp->pCol;
	
	ptCur->pCol=NULL;
	pTemp->pCol=ptCur;
}
struct MyIdentifyStruct *FindColBelongTab(struct MyIdentifyStruct *pTemp,char sTemp[])
{
	while(pTemp!=NULL){
		if(pTemp->pNext==NULL){
			pTemp=pTemp->pTab;
		...
		if(strcmp(pTemp->pNext->sSubType,"ASN")!=0){
			pTemp=pTemp->pTab;
			continue;
		}
		if(strcmp(sTemp,pTemp->pNext->sIdentify)==0)
			return pTemp;
		pTemp=pTemp->pTab;
	}
	return NULL;
}
void MarkTabNameColName(struct MyIdentifyStruct *ptHead)
{
	char *pc,sTemp[64];
	struct MyIdentifyStruct *pTemp,*ptCur,*ptPre,*pTab;

	while(ptHead!=NULL){
		pTemp=ptHead;
		ptHead=ptHead->pNext;
		
		if(pTemp->iType!=IDENTIFY_TYPE_OPER) continue;
		
		if(strcmp(pTemp->sSubType,"SEL")==0){
			ptCur=pTemp;
			while(ptCur!=NULL){
				if(strcmp(ptCur->sIdentify,"FROM")==0) break;
				ptCur=ptCur->pNext;
			}
			/*SELECT 没有匹配到FROM*/
			if(strcmp(ptCur->sIdentify,"FROM")!=0) return;
			ptPre=ptCur;
			ptCur=ptCur->pNext;
			
			if(ptCur==NULL) return;
			
			/*遇到子查询,直接跳过上面的部分*/
			if(strcmp(ptCur->sIdentify,"(")==0) continue;
			

			while(ptCur!=NULL){
				/*如果不是表名字则错误*/
				if(ptCur->iType!=IDENTIFY_TYPE_OTHER) break;
				
				
			if(strcmp(ptCur->sSubType,"TAB")!=0){

				strcpy(ptCur->sSubType,"TAB");
				ptCur->pHead=pTemp;
				AppendIdentifyToTabList(pTemp,ptCur);
			}	
			
				/*如果已经遇到结束则退出*/
				if(ptCur->pNext==NULL) break;
				...
				while(ptCur!=NULL){				
					...
					if(ptCur==NULL) break;
					/*跳过表名*/				
					if(ptCur->iType==IDENTIFY_TYPE_OTHER){
						strcpy(ptCur->sSubType,"ASN");
						/*跳过别名*/
						continue;
					}
					break;
			...
			/*循环，从pTemp,查找到ptCur,遇到 IDENTIFY_TYPE_OTHER,"SPL",
				则匹配第一个表，如果遇到"CPX",则截取pTab链表，匹配别名 */
			ptPre=pTemp;
			
			while(ptPre!=ptCur){
				if(ptPre->iType!=IDENTIFY_TYPE_OTHER){
					ptPre=ptPre->pNext;
					continue;
				}
				if(strcmp(ptPre->sSubType,"SPL")==0){
					AppendIdentifyToColList(pTemp->pTab,ptPre);
					ptPre=ptPre->pNext;
					continue;
				}
				if(strcmp(ptPre->sSubType,"CPX")==0){
				
					pc=strchr(ptPre->sIdentify,'.');
					strncpy(sTemp,ptPre->sIdentify,(pc-ptPre->sIdentify));
					sTemp[pc-ptPre->sIdentify]=0;
					
					pTab=pTemp->pTab;
					if((pTab=FindColBelongTab(pTemp->pTab,sTemp))==NULL)
						pTab=pTemp->pTab;
					
					AppendIdentifyToColList(pTab,ptPre);
					ptPre=ptPre->pNext;
					continue;
				}
				ptPre=ptPre->pNext;
		...
		else if (strcmp(pTemp->sSubType,"INS")==0){
			/*insert into tab_name (col1,col2,col3,...) values()
			  insert tab_name (col1,col2,col3...) select ...from */
			...
			if(strcmp(ptPre->sSubType,"TAB")!=0){
				/*将表名字挂入操作符后面*/
				strcpy(ptPre->sSubType,"TAB");
				ptPre->pHead=pTemp;
				AppendIdentifyToTabList(pTemp,ptPre);
			}
			...
			while(ptCur!=NULL){
				/*如果不是表名字则错误*/
				if(strcmp(ptCur->sIdentify,")")==0) break;
				
				if(ptCur->iType!=IDENTIFY_TYPE_OTHER){
					ptCur=ptCur->pNext;
					continue;
				}
				AppendIdentifyToColList(pTemp->pTab,ptCur);
				ptCur=ptCur->pNext;
			}
			
			ptHead=ptCur;
		}
		else if (strcmp(pTemp->sSubType,"DEL")==0){
			/*delete from tab_name where ...
			  delete tab_name where */
		...
		if(strcmp(ptPre->sSubType,"TAB")!=0){
		...
		else if (strcmp(pTemp->sSubType,"UPD")==0){
			/*update table_name set xx=yy where */
		...
		else if (strcmp(pTemp->sSubType,"CRT")==0||
			strcmp(pTemp->sSubType,"DRP")==0||
			strcmp(pTemp->sSubType,"TRC")==0||
			strcmp(pTemp->sSubType,"ALT")==0){
			...
			/*CREATE,DROP,TRUNCATE,ALT 后面无法匹配上表名 */
			if(ptCur->iType!=IDENTIFY_TYPE_OTHER) break;
...
int MyLexicalize(char sInput[],char sResult[])
{
	...
	struct MyIdentifyStruct *pTemp,*ptPre,*ptCur,*pPre,*pCur;

	DestroyList(pMyIdentifyHead);
	
	pMyIdentifyHead=NULL;
...
/*处理C语言注释*/
		if((iRet=LexCComment(sInput,&i))<0){iReturn=-1;break;}
		if(iRet==true) continue;
/*处理C++注释*/
		if((iRet=LexCppComment(sInput,&i))<0){iReturn=-1;break;}
		if(iRet==true) continue;

/*处理常量字符串*/
		if((iRet=MyLexConstString(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
//			l+=sprintf(sResult+l,"CONST=\t%s\n",sOutput);
			continue;
		}
/*处理常量十六进制数*/
		if((iRet=MyLexConstHexNum(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
//			l+=sprintf(sResult+l,"CONST=\t%s\n",sOutput);
			continue;
		}
/*处理常量二进制数*/
		if((iRet=MyLexConstBinNum(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
//			l+=sprintf(sResult+l,"CONST=\t%s\n",sOutput);
			continue;
		}
/*处理常量十进制数*/
		if((iRet=MyLexConstDecNum(sInput,&i,sOutput))<0) {iReturn=-1;break;}
		if(iRet==true){
//			l+=sprintf(sResult+l,"CONST=\t%s\n",sOutput);
			continue;
		}
/*处理标识符*/
		if((iRet=MyLexIdentify(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
			Upper(sOutput);
//			l+=sprintf(sResult+l,"IDENTIFY=%s\n",sOutput);
			
			if(AddMyIdentifyFlow(sOutput)<0){iReturn=-1;break;}

			continue;
		}
/*处理分割符*/
		if((iRet=MyLexSeperator(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
...
/*处理"+,-,*,/"算术符号*/
		if((iRet=MyLexMath(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
...
/*处理">,<,=,!,|,&"逻辑符号*/
		if((iRet=MyLexLogical(sInput,&i,sOutput))<0){iReturn=-1;break;}
		if(iRet==true){
//			l+=sprintf(sResult+l,"LOG=\t%s\n",sOutput);
	...
	pTemp=pMyIdentifyHead;

	while(pTemp!=NULL){
		if(pTemp->iType==IDENTIFY_TYPE_OTHER)
			l+=sprintf(sResult+l,"%s=\t%s\t----------------\n",pTemp->sSubType,pTemp->sIdentify);
		else
			l+=sprintf(sResult+l,"%s=\t%s\n",pTemp->sSubType,pTemp->sIdentify);
		if(l>=2000) break;
		pTemp=pTemp->pNext;
...
/*将SQL语句操作表和字段打印出来*/
	MarkTabNameColName(pMyIdentifyHead);
	
	pTemp=pMyIdentifyHead;
	
	while(pTemp!=NULL){
		if(pTemp->iType!=IDENTIFY_TYPE_OPER){
			pTemp=pTemp->pNext;
			continue;
		}
		
		l+=sprintf(sResult+l,"OPER=%s(",pTemp->sSubType);
		ptCur=pTemp->pTab;
		while(ptCur!=NULL){
			ptPre=ptCur;
			ptCur=ptCur->pTab;
			
			l+=sprintf(sResult+l,"TAB=%s(",ptPre->sIdentify);
			...
			while(pCur!=NULL){
				...
				if(pCur==NULL)
					l+=sprintf(sResult+l,"%s",pPre->sIdentify);
				else	l+=sprintf(sResult+l,"%s,",pPre->sIdentify);
				
				if(l>=2000) break;
			}
			
			l+=sprintf(sResult+l,"%s",")");
			
			if(ptCur==NULL)
				l+=sprintf(sResult+l,"%s",")");
			else	l+=sprintf(sResult+l,"%s",",");
