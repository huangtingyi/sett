...
##如果文件为空则继续
	...
	##检查是否重采
			filename_s=`echo $filename | cut -c 1-30`
			$PICKCHECK $source $filename_s
			ret=$?
			if [ $ret -lt 0 -o $ret -eq 255 ]; then
				printALERT "调用 $PICKCHECK $source $filename_s 错误 "
				exit 1;
			fi
			if [ $ret -gt 0 ]; then
	...
	##写采集数据库日志		
			msg=`$PICKLOG $source $size1 $filename_s $dealbegindate $dealenddate $seq $filetype $tacheid $appid`
			ret=$?
			if [ $ret -ne 0 ]; then
	...
	##采集临时文件改到输出文件			
			mv $localpath/$targetnametemp $localpath/$filename_s
			if [ $? -ne 0 ]; then
				printALERT "移动 mv $localpath/$targetnametemp $localpath/$filename_s 失败"
				exit 1;
			fi
			
			printNORMAL "$source 成功采集文件$filename_s 字节$size1"
			###用于监控信息的输出
			echo "时间：$dealenddate  $source 成功采集文件$filename_s 字节$size1 " >>$APPHOME/log/pick.sh.$source.log
		done
	done
