...
#include "../intf/biintf_nbr_supp.h"
#include "sett_supp.h"
...
int DealSettNbrStatUe(char sTableName[],char sCondition[],char sModule[],
	char sRoamDirection[],char msg[]);
...
int GenSQLCondition(char sModule[],char sStatMonth[],char sOppCarrier[],char sAreaCode[],
...
int data_search_bintree_f_sett_nbr_ue_e(void *pValue,void*pData)
{
...
void insert_bintree_f_sett_nbr_ue_e(void **ppData,void *pData)
{
       int iInFee=0,iOutFee=0;
        ...
        if((pTemp=(*ppData))==NULL){
                ...
                ERROR_EXIT(pTemp==NULL,"分配SettNbrStatUeStruct结构错误");
...
int XDealSettNbrBin2File(void **ppTemp)
{
	...
	while(p!=NULL){
		...
		memset((void*)&Temp,0,sizeof(struct SettNbrIntfFjbiStruct));
...
int InsertSettNbrIntfFjbi(struct SettNbrIntfFjbiStruct *p,int iInsertFlag)
{
        ...
        if(iFirstFlag==TRUE){
                iFirstFlag=FALSE;
                memset((void*)&TempOut,0,sizeof(struct SettNbrIntfFjbiStructOut));
...
int InitVisualFunc(char sFileType[])
{
	...
	else if(IsInMMS(sFileType)){
		XDealSettNbr=DealSettNbrStat;
		XDealRecord2intf=SCX_DealRecord2intf;
		
	}
	else{
		printf("未找到 %s 的业务类型.",sFileType);
		return -1;	
	}
	return 0;
}
...
int DealSettNbrStat(char sTableName[],char sCondition[],char sModule[],char sRoamDirection[],char msg[])
{
	...
	struct SettNbrStatStruct Temp;
        ...
        memset((void*)&TempIn,0,sizeof(struct SettNbrStatStructIn));
	...
	while(EGetSettNbrStatToStruct(&Temp,&TempIn)){
		...
		memset((void*)&TempOut,0,sizeof(struct SettNbrIntfFjbiStruct));
...
int XDealSettNbrUeBin2File(struct SettNbrStatUeStruct *p)
{
	int l=0;
	char sRecord[1024];
	...
	l=0;
	memset(sRecord,0,sizeof(sRecord));	
	memset((void*)&Temp,0,sizeof(struct SettNbrStatUeStruct));
	
	iTotalCnt+=p->iSettCnt*1.0/10000.0;
	iTotalDuration+=p->iDuration*1.0/10000.0;
	iTotalFeeDuration+=p->iSettDuration*1.0/10000.0;
	iTotalInFee+=p->iSettIncome*1.0/10000.0;
	iTotalOutFee+=p->iSettExpenses*1.0/10000.0;
	
	if(strcmp(p->sSvcType,"VIT")==0&&
		strcmp(p->sVisitAreaCode,"")==0)
		strcpy(p->sVisitAreaCode,p->sSettCity);

/*增加去掉号码后的统计数据，插到 pRootchk 树中*/
	memcpy((void*)&Temp,(void*)p,sizeof(struct SettNbrStatUeStruct));
	strcpy(Temp.sSettNumber,"ALL");

	if(InsertBin(&pRootchk,(void *)&Temp,
		data_search_bintree_f_sett_nbr_ue_e,
		insert_bintree_f_sett_nbr_ue_e)<0){
		WriteAlertMsg("加载数据到内存树失败");
		return -1;
	}		
	
	l+=sprintf(sRecord+l,"|%s|"	,p->sSettDate);
	l+=sprintf(sRecord+l,"0%s|"	,p->sSettCity);
	l+=sprintf(sRecord+l,"%s|"	,p->sSettNumber);
	l+=sprintf(sRecord+l,"0%s|"	,p->sHomeAreaCode);
	l+=sprintf(sRecord+l,"0%s|"	,p->sOtherAreaCode);
	l+=sprintf(sRecord+l,"0%s|"	,p->sVisitAreaCode);
	l+=sprintf(sRecord+l,"%s|"	,p->sHomeCompCode);
	l+=sprintf(sRecord+l,"%s|"	,p->sOtherCompCode);
	l+=sprintf(sRecord+l,"%s|"	,p->sCallDirection);
	l+=sprintf(sRecord+l,"%s|"	,p->sProdID);		
	l+=sprintf(sRecord+l,"%d|"	,p->iSettIncome);
	l+=sprintf(sRecord+l,"%d|"	,p->iSettExpenses);
	l+=sprintf(sRecord+l,"%d|"	,p->iSettDuration);
	l+=sprintf(sRecord+l,"%d|"	,p->iDuration);
	l+=sprintf(sRecord+l,"%d|"	,p->iSettCnt);
	l+=sprintf(sRecord+l,"%s|"	,p->sSvcType);
	l+=sprintf(sRecord+l,"%s|"	,p->sHomeNetType);
	l+=sprintf(sRecord+l,"%s|"	,p->sOtherNetType);
	l+=sprintf(sRecord+l,"%s|"	,p->sSettFlag);
	l+=sprintf(sRecord+l,"%s|"	,p->sReserve1);
	l+=sprintf(sRecord+l,"%s|\r\n"	,p->sReserve2);
...
int DealSettNbrStatUe(char sTableName[],char sCondition[],char sModule[],char sRoamDirection[],char msg[])
{
	int iCnt=0;
	struct SettNbrStatUeStruct Temp;
	...
	while(EGetSettNbrStatUeToStruct(&Temp,&TempIn)){
		...
		DealAreaCode(Temp.sSettCity);
		DealAreaCode(Temp.sHomeAreaCode);
		DealAreaCode(Temp.sOtherAreaCode);

		if(XDealSettNbrUeBin2File(&Temp)<0)return -1;
	...
	char sRoamDirection[],char sCondition[])
{
	...
	if(IsInSTP(sModule)){
		if(strcmp(sOppCarrier,"CT")==0){
			...
			strcat(sCondition," WHERE SETT_CITY='");
			strcat(sCondition,sAreaCode);
			strcat(sCondition,"'");
			
			strcat(sCondition," AND SETT_DATE='");
			strcat(sCondition,sStatMonth);
			strcat(sCondition,"'");
			
			strcat(sCondition," AND IN_COMP IN ('CT','CTC')");
	...
	if(IsInSTT(sModule) || IsInSTP(sModule)){
		...
		if(strcmp(sOppCarrier,"CT")==0||strcmp(sOppCarrier,"CM")==0||strcmp(sOppCarrier,"CNC")==0){
			...
			strcat(sCondition," AND IN_COMP='");
			strcat(sCondition,sOppCarrier);
			strcat(sCondition,"'");
		}
		else if(strcmp(sOppCarrier,"CRC")==0||strcmp(sOppCarrier,"CTC")==0){

			strcat(sCondition," WHERE SETT_DATE='");
			strcat(sCondition,sStatMonth);
			strcat(sCondition,"'");
 ...
 /*       	if(IsInSTT(sModule) && strcmp(sOppCarrier,"CT")==0){
	...
	else if(IsInSCX(sModule) || IsInSMS(sModule) ||IsInMMS(sModule)){
		/*
...
int main(int argc,char *argv[])
{
	...
	/*获取输出数据的SQL条件*/
        if(IsInSTT(sModule) || IsInSTP(sModule))
        	GenSQLCondition(sModule,sStatMonth,sOppCarrier,sAreaCode,"",sCondition);
	else if(IsInSCX(sModule) || IsInSMS(sModule) || IsInMMS(sModule))
        	GenSQLCondition(sModule,sStatMonth,"","","",sCondition);
	else if(IsInVIT(sModule))
        	GenSQLCondition(sModule,sStatMonth,"","",sRoamDirection,sCondition);
	else if(IsInTLL(sModule) || IsInTLP(sModule))
        	GenSQLCondition(sModule,sStatMonth,"","","",sCondition);
        else{
        	printf("获取不到SQL条件.\n");
        ...
        if(DealSettNbrStatUe(sTableName,sCondition,sModule,sRoamDirection,msg)<0){
        	printf("%s",msg);
        	WriteAlertMsg(msg);
        	return -1;
        }
