...
*/	
	...
	po->iSettDuration=atoi(Temp.sStlDuration);		/* 结算时长 */
	po->iDuration=atoi(Temp.sDuration);		/* 通话时长 */
	po->iSettCnt=Temp.iSettCnt;		/* 通话次数 */
...
int main(int argc,char **argv)
{
	...
	while(iGroupID<=iEnd){
		...
		WriteProcMsg("统计表 %s 分组 %02d 完成并入库到表%s.",sSouTableName,iGroupID,STATCTL.sOutTableName);
		iGroupID++;
	}
	if(IsInTLL(STATCTL.InType)){
