...
/*-----20111216添加通用读写表支持---------*/
	l=RegShmTabFun(IInitTabTemplate,
		sizeof(struct TabTemplateStruct),
		"TAB_TEMPLATE",pptHead);
	if(l<0) return -1;
	l=RegShmTabFun(IInitTabColumns,
		sizeof(struct TabColumnsStruct),
		"TAB_COLUMNS",pptHead);
	if(l<0) return -1;
	l=RegShmTabFun(IInitTabTemplateReal,
		sizeof(struct TabTemplateRealStruct),
		"TAB_TEMPLATE_REAL",pptHead);
	if(l<0) return -1;
...
int IInitTabTemplate(void **pptHead)
{
	return InitTabTemplate((struct TabTemplateStruct**)pptHead);	
}
int IInitTabColumns(void **pptHead)
{
	return InitTabColumns((struct TabColumnsStruct**)pptHead);	
}
int IInitTabTemplateReal(void **pptHead)
{
	return InitTabTemplateReal((struct TabTemplateRealStruct**)pptHead);	
}
