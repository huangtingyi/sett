...
void GetPrepErrScale(char sBusinessModule[],double *iErrScale)
{
	...
	else if(strcmp(sBusinessModule,"GSM")==0)	*iErrScale=0.00028;
...
{
	if(strcmp(sHost,"ZHJF")==0){
		if(strcmp(sBusinessModule,"CDMA")==0)
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID LIKE '__DC__'\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);

		else if(strcmp(sBusinessModule,"GSM")==0)
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID LIKE '__DG__'\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);
			...
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID = 'CDMA1X'\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);

		else if(strcmp(sBusinessModule,"DX_CDMA")==0)
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID IN ('LZPC01','LZSM01','LZST01')\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);
			...
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID IN ('LZPG01','LZSM01','LZST01')\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);
	...
	if(strcmp(sHost,"ZHJS")==0){
		if(strcmp(sBusinessModule,"WJ")==0)
			sprintf(sCondition,"\n\
				WHERE SOURCE_ID NOT IN ('LZSS01','LZSS03')\n\
				AND to_char(DATA_DATE,'yyyymmdd')='%s'",
				sDataDate
				);
		else{
...
int main(int argc,char *argv[])
{
	...
	if(argc!=5){
		...
		printf("Caller:	kpi_nmsintf4gs ZHJF PREP CDMA .\n" );
		return -1;
	}
	...
	if(strcmp(sHost,"ZHJF")!=0&&strcmp(sHost,"ZHJS")!=0){
		printf("系统模块不正确，请重新输入.\n");
		printf("目前包括系统模块：ZHJF/ZHJS.\n");
		return -1;
	}
	...
	if(strcmp(sBusinessModule,"CDMA")!=0&&
		...
		printf("目前包括业务模块：CDMA/GSM/WJ/1X/DX_CDMA/DX_GSM.\n");
		return -1;
	}
	...
	if(GenIntfFile(sHost,sProcModule,sBusinessModule,fp)<0){
		WriteProcPrint("生成KPI网管接口文件%s错误,sFileName.\n",
			sHost,sBusinessModule);
		return -1;
	}
