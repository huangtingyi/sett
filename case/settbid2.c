...
int SetGroupKey(struct SettItemStruct  *pi,char sExprName[])
{
	int iLatnID=-1,iOppCarrierID=-1,iSettCycleID=-1;
	int iAcctItemTypeID=-1,iCharge=0;
	char sSettItemDate[15],sUserType[10],sSourceID[31];
	

	set_int_value("in_latn_id",pExprBuf,pi->iLatnID);
	set_int_value("in_acct_item_type_id",pExprBuf,pi->iAcctItemTypeID);
	set_int_value("in_charge",pExprBuf,pi->iCharge);
	set_str_value("in_user_type",pExprBuf,pi->sUserType);
	set_str_value("in_sett_item_date",pExprBuf,pi->sSettItemDate);
	set_int_value("in_opp_carrier_id",pExprBuf,pi->iOppCarrierID);
	set_int_value("in_sett_cycle_id",pExprBuf,pi->iSettCycleID);
	set_str_value("in_source_id",pExprBuf,pi->sSourceID);

	if(ExecCode(pExprBuf)<0){
		printf("执行公式%s失败.\n",sExprName);
		return -1;
	}
	get_int_value("set_latn_id",pExprBuf,&iLatnID);
	get_int_value("set_acct_item_type_id",pExprBuf,&iAcctItemTypeID);
	get_int_value("set_charge",pExprBuf,&iCharge);
	get_str_value("set_user_type",pExprBuf,sUserType);
	get_str_value("set_sett_item_date",pExprBuf,sSettItemDate);
	get_int_value("set_opp_carrier_id",pExprBuf,&iOppCarrierID);
	get_int_value("set_sett_cycle_id",pExprBuf,&iSettCycleID);
	get_str_value("set_source_id",pExprBuf,sSourceID);
	
	if(iLatnID!=-1&&(pi->iLatnID-iLatnID)!=0){
		if(iLatnID==-256)pi->iLatnID=-1;
		else pi->iLatnID=iLatnID;
	}
	if(strcmp(sSettItemDate,"")!=0&&
		strcmp(pi->sSettItemDate,sSettItemDate)!=0)
		strcpy(pi->sSettItemDate,sSettItemDate);
...
/*编译公式*/
	...
	sprintf(TempIn.sTableName,"SETT_ITEM_SP");
	if(iLatnID==-1){
	...
	while(EGetSettItemToStruct(&Temp,&TempIn)){
		...
		if(iLatnID==-1){
			SetGroupKey(&Temp,sExprName);
		}
		/**
		strcpy(Temp.sSourceID,"000000");
		已经在日结公式中设置了，只有找不到的情况下才设置这个东西..
		**/
	...
	/**删除过去统计结果**/
	if(iLatnID==-1){
		sprintf(statement,"DELETE FROM SETT_ITEM_MONTH \n\
			WHERE SETT_ITEM_DATE>=to_date('%s','yyyymmddhh24miss') AND\n\
				SETT_ITEM_DATE<=to_date('%s','yyyymmddhh24miss') \n\
			",sDealBeginDate,sDealEndDate);
	}
	...
	if(iRet<0){
		 printf("删除SETT_ITEM_MONTH记录错误.\n");
		 WriteErrStackAlert();
		 return -1;
	}
	/**输出数据到月帐表**/
        bzero((void*)&TempOut,sizeof(struct SettItemStructOut));
        strcpy(TempOut.sTableName,"SETT_ITEM_MONTH");
